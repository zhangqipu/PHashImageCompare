...
//    UIImageWriteToSavedPhotosAlbum(img, nil, nil, NULL);
    ...
    NSMutableArray *vals = [self allocateTwoDimension];
    
    for (int x = 0; x < img.size.width; x++) {
        for (int y = 0; y < img.size.height; y++) {
            vals[x][y] = @([self getBlueWithImage:img atPoint:CGPointMake(x, y)]);
        }
    }

    NSArray *dctVals = [self applyDCT:vals];
    double total = 0;
    
    for (int x = 0; x < _smallerSize; x++) {
        for (int y = 0; y < _smallerSize; y++) {
            total += [dctVals[x][y] doubleValue];
        }
    }
    
    total -= [dctVals[0][0] doubleValue];
    ...
    for (int x = 0; x < _smallerSize; x++) {
        for (int y = 0; y < _smallerSize; y++) {
            if (x != 0 && y != 0) {
                [hash appendString:[dctVals[x][y] doubleValue] > avg ? @"1":@"0"];
            }
        }
    }
